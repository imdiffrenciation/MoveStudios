...
const MediaCard = ({
  ...
  return <Card className="group overflow-hidden transition-all duration-500 hover:shadow-2xl hover:shadow-primary/20 cursor-pointer border-border" onMouseEnter={handleMouseEnter} onMouseLeave={() => setIsHovered(false)} onClick={() => onMediaClick(item)}>
      {/* Media Container */}
      <div className="relative aspect-square overflow-hidden bg-secondary">
        {item.type === 'image' ? <>
            ...
            <img src={item.url} alt={item.title} className={`w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 ${imageLoaded ? 'opacity-100' : 'opacity-0'}`} onLoad={() => setImageLoaded(true)} />
